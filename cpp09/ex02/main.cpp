...
std::vector<int> sort_algo(std::vector<int> &numbers, int level)
{
    ...
    // {
    //     std::cerr << "DEBUG: assembled vector size (" << new_vec.size() << ") != input size (" << numbers.size() << ") - falling back to std::sort" << std::endl;
    //     std::vector<int> fallback = numbers;
...
std::deque<int> generate_jacob_sizes(int limit)
{
...
std::deque<int> sort_algo(std::deque<int> &numbers, int level)
{
        ...
        for (size_t seq_idx = 1; seq_idx < jacob_sequence.size(); ++seq_idx)
        {
            ...
            if (counter > 0 || (counter == 0 && jacob_flag[seq_idx] == 0))
            {
                ...
                while (left < right)
                {
                    size_t middle = left + (right - left) / 2;
                    std::cout << "left = " << left << " middle = " << middle << " right = " << right << std::endl;
                    // protect against empty inner deques in main
                    if (main[middle].empty() || pend[pend_index].back() < main[middle].back())
                        right = middle;
                    else
                        left = middle + 1;
    ...
    // if (new_vec.size() != numbers.size())
    // {
    //     std::cerr << "DEBUG: assembled deque size (" << new_vec.size() << ") != input size (" << numbers.size() << ") - falling back to std::sort" << std::endl;
    //     std::deque<int> fallback = numbers;
    //     std::sort(fallback.begin(), fallback.end());
    //     return fallback;
    // }
    return new_vec;
}
bool is_sorted(std::deque<int> &numbers)
{
    for (size_t i = 0; i < numbers.size() - 1; i++)
    {
        if (numbers[i] > numbers[i + 1])
...
std::deque<int> pairing(std::deque<int> &numbers, int level)
{
